...
class UserClass extends database {
    ...
    /*function dbPatch(){
    	$result_update = false;
		$sql = "SELECT DISTINCT `bed_no`, ( SELECT bed_name FROM `bed_details` WHERE bed_name = (SELECT bed_name FROM `bed_details` WHERE id = bed_no) AND id < 175 ) AS 'replaced_bed_no',( SELECT id FROM `bed_details` WHERE bed_name = (SELECT bed_name FROM `bed_details` WHERE id = bed_no) AND id < 175 ) AS 'replaced_bed_id' FROM `patient_details` WHERE `bed_no` IN (SELECT id FROM `bed_details` WHERE `id` > 175 AND `bed_del` = 0)";
		$result = $this->conn->query($sql); 
		if (mysqli_num_rows($result) > 0) {
            while ($row = $result->fetch_assoc()) {
            	echo $row['bed_no']." >> ".$row['replaced_bed_id']."</br>";
            	$count = array('bed_no' => $row['replaced_bed_id']);
            	$result_update = $this->dbRowUpdate("patient_details", $count, '`bed_no`=' . $row['bed_no']);
            }
      }
      if($result_update) {
      	$result_delete = $this->dbRowDelete("bed_details", '`id`> 175');
      	if($result_delete){
      		$count = array('bed_del' => '0');
            $result_update = $this->dbRowUpdate("bed_details", $count, '`bed_del`=1');
      	}
      }
    }*/
